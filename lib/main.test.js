...
ava_1.default("unbundle creates a stable directory name", async (t) => {
    ...
    try {
        ...
        await main_1.unbundleDatabase(dbFile);
...
ava_1.default("running a basic query", async (t) => {
    ...
    try {
        ...
        await main_1.runQuery("codeql", "javascript", dbDir, "import javascript\nfrom File f select f", "a/b");
        t.true(fs_1.default
...
ava_1.default("we can download a database", async (t) => {
    process.env.RUNNER_TEMP = "/tmp";
    const dbZip = await main_1.downloadDatabase(process.env.TOKEN || "", "dsp-testing/qc-demo-github-certstore", "go");
    t.true(fs_1.default.existsSync(dbZip));
});
