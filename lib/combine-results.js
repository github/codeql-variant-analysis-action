...
async function run() {
    try {
        ...
        const resultsIndex = await Promise.all(resultArtifacts.map(async function (response) {
            const resultIndexItem = {};
            resultIndexItem.nwo = fs_1.default.readFileSync(path_1.default.join(response.downloadPath, "nwo.txt"), "utf-8");
            resultIndexItem.id = response.artifactName;
            resultIndexItem.results_count = parseInt(fs_1.default.readFileSync(path_1.default.join(response.downloadPath, "resultcount.txt"), "utf-8"), 10);
            resultIndexItem.bqrs_file_size = fs_1.default.statSync(path_1.default.join(response.downloadPath, "results.bqrs")).size;
            if (fs_1.default.existsSync(path_1.default.join(response.downloadPath, "results.sarif"))) {
                resultIndexItem.sarif_file_size = fs_1.default.statSync(path_1.default.join(response.downloadPath, "results.sarif")).size;
            }
