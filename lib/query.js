...
async function getArtifactContentsForUpload(runQueryResult) {
    ...
    if (runQueryResult.sarifFilePath) {
        const sarifFileContents = fs_1.default.readFileSync(runQueryResult.sarifFilePath, "utf-8");
        zip.file("results.sarif", sarifFileContents);
    }
    else {
        const bqrsFileContents = fs_1.default.readFileSync(runQueryResult.bqrsFilePath, "utf-8");
