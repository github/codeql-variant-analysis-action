...
async function run() {
    ...
    const variantAnalysisId = parseInt((0, core_1.getInput)("variant_analysis_id"));
    ...
    try {
        ...
        for (const repo of repos) {
            ...
            if (liveResults) {
                await (0, gh_api_client_1.setVariantAnalysisFailed)(variantAnalysisId, repo.id, error.message);
            }
    ...
    for (const repo of repos) {
        ...
        try {
            ...
            if (liveResults) {
                await (0, gh_api_client_1.setVariantAnalysisRepoInProgress)(variantAnalysisId, repo.id);
            }
            ...
            if (liveResults) {
                ...
                await (0, gh_api_client_1.setVariantAnalysisRepoSucceeded)(variantAnalysisId, repo.id, runQueryResult.sourceLocationPrefix, runQueryResult.resultCount, runQueryResult.databaseSHA || "HEAD");
            }
            await uploadResultsAsActionsArtifacts(runQueryResult, artifactClient, repo);
        }
        catch (error) {
            (0, core_1.setFailed)(error.message);
            await uploadError(error, repo, artifactClient);
...
async function uploadResultsAsActionsArtifacts(runQueryResult, artifactClient, repo) {
    const filesToUpload = [
...
async function uploadQueryResultForRepo(variantAnalysisId, repo, runQueryResult) {
    // Get policy for artifact upload
    const policy = await (0, gh_api_client_1.getPolicyForRepoArtifact)(variantAnalysisId, repo.id, runQueryResult.sarifFileSize || runQueryResult.bqrsFileSize);
    // Use Azure client for uploading to Azure Blob Storage
    const fileToUpload = runQueryResult.sarifFilePath || runQueryResult.bqrsFilePath;
    await (0, azure_client_1.uploadArtifact)(policy, fileToUpload);
}
