    ...
    for (const repo of repos) {
        try {
            ...
            if (repo.downloadUrl) {
                ...
                console.log("Getting database");
                dbZip = await (0, codeql_1.downloadDatabase)(repo.id, repo.nwo, language, repo.token, repo.pat);
            }
            // 2. Run the query
...
async function uploadError(error, repo, artifactClient) {
    ...
    const errorFile = path_1.default.join("errors", "error.txt");
    fs_1.default.appendFileSync(errorFile, error.message);
    const nwoFile = path_1.default.join("errors", "nwo.txt");
    fs_1.default.writeFileSync(nwoFile, repo.nwo);
