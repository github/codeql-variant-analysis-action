...
async function run() {
    ...
    try {
        const query = (0, core_1.getInput)("query");
        const queryPackUrl = (0, core_1.getInput)("query_pack_url");
        ...
        for (const repo of repos) {
            ...
            if (queryPackUrl !== "") {
                ...
                const queryPackArchive = await (0, download_database_1.downloadDatabaseFile)(queryPackUrl, "query_pack.tar.gz");
                queryPack = await (0, tool_cache_1.extractTar)(queryPackArchive);
            }
            // 2. Run the query
            console.log("Running query");
            await (0, codeql_1.runQuery)(codeql, language, dbZip, repo.nwo, query, queryPack);
