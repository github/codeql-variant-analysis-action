...
async function run() {
    ...
    try {
        ...
        for (const repo of repos) {
            ...
            // 1. Use the GitHub API to download the database using token
            const dbZip = await (0, codeql_1.downloadDatabase)(repo.id, language, repo.token, repo.pat);
            // 2. Run the query
            await (0, codeql_1.runQuery)(codeql, language, dbZip, query, repo.nwo);
        ...
        (0, fs_1.appendFileSync)(errorFile, err.message); // TODO: Include information about which repository produced the error
