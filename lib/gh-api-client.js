...
async function updateVariantAnalysisStatus(controllerRepoId, variantAnalysisId, repoId, data) {
    ...
    if (response.status !== 204) {
        console.log(`Request to ${url} returned status code ${response.status}:
      ${JSON.stringify(response.data)}`);
        throw new Error(`Error while setting variant analysis as "${data.status}". Status code: ${response.status}`);
    }
}
async function getPolicyForRepoArtifact(controllerRepoId, variantAnalysisId, repoId, artifactSize) {
    ...
    if (response.status !== 201) {
        console.log(`Request to ${url} returned status code ${response.status}:
      ${JSON.stringify(response.data)}`);
        throw new Error(`Error while getting policy for artifact. Status code: ${response.status}`);
    }
