...
(0, ava_1.default)("getting the commit SHA for a database", async (t) => {
    // Note: db-metadata-only.zip is not a real database.
    // It doesn't contain source code, only a codeql-database.yml file.
    const testDbZip = path_1.default.resolve("testdata/db-metadata-only.zip");
    const tmpDir = fs_1.default.mkdtempSync("tmp");
    const cwd = process.cwd();
    process.chdir(tmpDir);
    try {
        // Test our fake database. This "database" has a commit SHA in its metadata.
        await (0, exec_1.exec)("codeql", ["database", "unbundle", testDbZip, "--name=fakeDb"]);
        const sha1 = (0, codeql_1.getDatabaseSHA)("fakeDb");
