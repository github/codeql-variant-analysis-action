...
ava_1.default("unbundle creates a stable directory name", async (t) => {
    ...
    try {
        ...
        await codeql_1.unbundleDatabase(dbFile);
...
ava_1.default("running a basic query", async (t) => {
    ...
    try {
        ...
        await codeql_1.runQuery("codeql", "javascript", dbDir, "import javascript\nfrom File f select f", "a/b");
        t.true(fs_1.default
        ...
        t.is(fs_1.default.readFileSync(path_1.default.join("results", "nwo.txt"), "utf-8"), "a/b");
    }
    finally {
        process.chdir(cwd);
        await io_1.rmRF(tmpDir);
