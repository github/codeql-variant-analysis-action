...
async function createResultIndex(resultArtifacts, errorArtifacts) {
    const resultArtifactPromises = resultArtifacts.map(async function (response) {
    ...
    const errorArtifactPromises = errorArtifacts.map(async function (response) {
        const nwo = fs_1.default.readFileSync(path_1.default.join(response.downloadPath, "nwo.txt"), "utf-8");
    ...
    return await Promise.all(resultArtifactPromises.concat(errorArtifactPromises));
}
