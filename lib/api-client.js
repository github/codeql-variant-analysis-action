...
async function updateVariantAnalysisStatus(variantAnalysisId, repoId, data) {
    ...
    if (response.message.statusCode !== 204) {
      ...
      ${await response.readBody()}`);
        throw new Error(`Error while setting variant analysis as "${status}". Status code: ${response.message.statusCode}`);
    }
}
async function getPolicyForRepoArtifact(variantAnalysisId, repoId, artifact_size) {
    const data = {
        name: "results.zip",
    ...
    if (response.message.statusCode !== 201) {
        console.log(`Request to ${url} returned status code ${response.message.statusCode}:
      ${await response.readBody()}`);
        throw new Error(`Error while getting signed URL repo artifact". Status code: ${response.message.statusCode}`);
    }
