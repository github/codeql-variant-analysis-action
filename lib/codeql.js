...
async function getQueryMetadata(codeql, query) {
    const queryMetadata = await (0, exec_1.getExecOutput)(codeql, [
        "resolve",
        "metadata",
        "--format=json",
        query,
    ...
    if (queryMetadata.exitCode !== 0) {
        throw new Error(`Unable to run codeql resolve metadata. Exit code: ${queryMetadata.exitCode}`);
    }
    return JSON.parse(queryMetadata.stdout);
}
