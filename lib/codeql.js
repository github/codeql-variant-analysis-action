...
exports.getRemoteQueryPackDefaultQuery = exports.getDatabaseMetadata = exports.getSarifResultCount = exports.injectVersionControlInfo = exports.generateSarif = exports.getSarifOutputType = exports.getBqrsInfo = exports.downloadDatabase = exports.runQuery = void 0;
...
async function runQuery(codeql, database, nwo, queryPack) {
    ...
    const databaseSHA = (_c = (_b = dbMetadata.creationMetadata) === null || _b === void 0 ? void 0 : _b.sha) === null || _c === void 0 ? void 0 : _c.toString();
    ...
    const queryMetadata = await getQueryMetadata(codeql, await getRemoteQueryPackDefaultQuery(codeql, queryPack));
    const sourceLocationPrefix = await getSourceLocationPrefix(codeql);
    const sarifOutputType = getSarifOutputType(queryMetadata, compatibleQueryKinds);
    let resultCount;
    let sarifFilePath;
    let sarifFileSize;
    if (sarifOutputType !== undefined) {
        const sarif = await generateSarif(codeql, bqrsFilePath, nwo, sarifOutputType, databaseName, sourceLocationPrefix, databaseSHA);
    ...
    (0, query_run_metadata_1.writeQueryRunMetadataToFile)(metadataFilePath, nwo, resultCount, databaseSHA, sourceLocationPrefix);
    return {
        resultCount,
        databaseSHA,
...
async function getQueryMetadata(codeql, query) {
    const queryMetadataOutput = await (0, exec_1.getExecOutput)(codeql, [
        "resolve",
        "metadata",
        "--format=json",
        query,
...
function getSarifOutputType(queryMetadata, compatibleQueryKinds) {
    ...
    if (queryKind === "path-problem" &&
        compatibleQueryKinds.includes("PathProblem")) {
        return "path-problem";
    }
    else if (queryKind === "problem" &&
...
function getDatabaseMetadata(database) {
    try {
        return (0, yaml_1.parseYamlFromFile)(path_1.default.join(database, "codeql-database.yml"));
    }
    catch (error) {
