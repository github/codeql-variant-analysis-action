...
async function runQuery(codeql, database, nwo, queryPack) {
    ...
    if (sarifOutputType !== undefined) {
        const sarif = await generateSarif(codeql, bqrsFilePath, nwo, sarifOutputType, databaseName, sourceLocationPrefix, dbMetadata.creationMetadata?.sha);
    ...
    (0, query_run_metadata_1.writeQueryRunMetadataToFile)(metadataFilePath, nwo, resultCount, dbMetadata.creationMetadata?.sha, sourceLocationPrefix);
    return {
        resultCount,
