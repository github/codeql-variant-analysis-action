...
exports.getQueryPackDefaultQuery = exports.getDatabaseSHA = exports.getBqrsInfo = exports.runQuery = exports.downloadDatabase = void 0;
 ...
 */
function getQueryPackDefaultQuery(queryPack) {
    let metadata;
    try {
        metadata = yaml.load(fs_1.default.readFileSync(path_1.default.join(queryPack, "qlpack.yml"), "utf8"));
    }
    catch (error) {
        console.log(`Unable to read qlpack.yml: ${error}`);
        return;
    }
    if (metadata === null || metadata === void 0 ? void 0 : metadata.defaultSuite) {
        const entry = metadata.defaultSuite.find((elem) => elem.query);
        if (entry === null || entry === void 0 ? void 0 : entry.query) {
            return entry.query;
        }
    }
    console.log("Query pack does not contain a default query suite with a query.");
    return;
}
exports.getQueryPackDefaultQuery = getQueryPackDefaultQuery;
