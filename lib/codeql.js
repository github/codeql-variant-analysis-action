...
async function downloadDatabase(signedAuthToken, repoId, language, pat) {
    const authHeader = pat === undefined ? `RemoteAuth ${signedAuthToken}` : `token ${pat}`;
    return tool_cache_1.downloadTool(`https://api.github.com/repositories/${repoId}/code-scanning/codeql/databases/${language}`, `${repoId}.zip`, authHeader);
}
