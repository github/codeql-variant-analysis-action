...
async function runQuery(codeql, language, database, query, nwo) {
    ...
    fs.writeFileSync(path.join(queryDir, "qlpack.yml"), `name: queries
version: 0.0.0
libraryPathDependencies: codeql-${language}`);
    ...
    const sourceLocationPrefix = JSON.parse((await getExecOutput(codeql, ["resolve", "database", database])).stdout).sourceLocationPrefix;
...
async function downloadDatabase(token, nwo, language) {
    return downloadTool(`https://api.github.com/repos/${nwo}/code-scanning/codeql/databases/${language}`, undefined, `token ${token}`);
}
