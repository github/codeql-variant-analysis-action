...
test("getDefaultCliVersion with valid features", (t) => {
  const features = {
  ...
  t.is(getDefaultCliVersion(features), "2.17.1");
});

test("getDefaultCliVersion without version features", (t) => {
  const features = {
    /* eslint-disable @typescript-eslint/naming-convention -- names are from an API response */
    analysis_summary_v2_enabled: true,
    upload_failed_sarif_enabled: true,
  ...
  t.is(getDefaultCliVersion(features), undefined);
});
