...
class InterfaceAndClassNodeParser {
    ...
    getProperties(node, context) {
        ...
        const properties = node.members
            ...
            .filter((member) => (0, modifiers_1.isPublic)(member) && !(0, modifiers_1.isStatic)(member) && member.type && !(0, isHidden_1.isNodeHidden)(member))
            .map((member) => new ObjectType_1.ObjectProperty(this.getPropertyName(member.name), this.childNodeParser.createType(member.type, context), !member.questionToken))
            .filter((prop) => {
